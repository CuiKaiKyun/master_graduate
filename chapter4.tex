...
\end{equation}
根据高斯-牛顿法，在第 $k$ 次迭代时，需计算误差向量关于参数的雅可比矩阵 $\bm{J}_T^{(k)} \in \mathbb{R}^{n \times 2}$，其元素为：
\begin{equation}
    \bm{J}_T^{(k)} = 
    \begin{bmatrix}
...
高斯-牛顿迭代法中的迭代步长 $\bm{\delta}_T^{(k)} = [\Delta C_{TAmp}, \Delta C_{TExp}]^{T}$ 由正规方程确定：
\begin{equation}
    \bm{\delta}_T^{(k)} = -\left( (\bm{J}_T^{(k)})^{T} \bm{J}_T^{(k)} \right)^{-1} (\bm{J}_T^{(k)})^{T} \bm{E}_{TGE}^{(k)}.
\end{equation}
参数更新公式为：
\begin{equation}
    \bm{\theta}_T^{(k+1)} = \bm{\theta}_T^{(k)} + \bm{\delta}_T^{(k)},
\end{equation}
其中 $\bm{\theta}_T^{(k)} = [C_{TAmp}^{(k)}, C_{TExp}^{(k)}]^{T}$。该迭代算法在收敛点附近具有二次收敛速度。
