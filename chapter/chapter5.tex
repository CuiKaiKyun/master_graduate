...
进一步地，当无人机在开放环境中执行大机动飞行或遭遇阵风扰动时，涵道风扇的侧向力、陀螺力矩及反扭矩襟翼平衡效应等呈现强烈的非线性耦合特性，单纯依赖串级PID控制器难以实现精确补偿。得益于第三章基于扩展卡尔曼滤波器在线辨识得到的关键气动参数，如 \(K_{sta}\)、\(K_{cv}\)，本章进一步设计了非线性动态逆控制器。NDI控制器通过直接对系统动力学模型求逆，将原始非线性系统映射为期望的线性解耦形式，从而主动抵消非线性耦合与参数不确定性带来的不利影响。仿真环境下，对比串级PID控制器与NDI控制器在姿态阶跃响应中的表现，验证了NDI控制器在响应速度、抗耦合能力及动态一致性方面的显著优势。
...
为验证本文所提出的地面效应补偿算法在实际飞行环境中的有效性与鲁棒性，本小节以涵道式无人机实验样机为对象，开展近地面抗扰控制实飞实验。实验在室内动作捕捉系统的有效测量空间内进行，通过对比是否部署地面效应补偿算法时无人机在近地面端的高度位置阶跃响应与偏航角阶跃响应，定量评估推力补偿与舵面偏转力补偿对控制性能的改善效果。
...
高度位置阶跃响应实验结果表明：本文所提出的地面效应推力补偿算法能够有效克服强地效区推力增益带来的不利影响，显著改善近地面高度控制的稳态精度与动态品质，使无人机在 $0.1\,\mathrm{m}$ 与 $0.3\,\mathrm{m}$ 高度上展现出高度一致的动态特性，为安全起降与低空悬停提供了可靠保障。
% \subsection{高度位置阶跃响应实验数据分析}
% 经整理，实验数据如下。
...
综合上述分析，偏航角阶跃响应实验结果表明：本文所提出的舵面偏转力补偿算法能够有效克服地面效应对偏航通道的不利影响，显著提升近地面姿态控制的响应速度与稳态精度。引入补偿后，无人机在 \(h = 100\,\mathrm{mm}\) 与 \(h = 200\,\mathrm{mm}\) 两个高度上展现出高度一致的动态特性，验证了补偿模型在不同地效强度下的有效性与鲁棒性。
...
本小节针对涵道式无人机近地面飞行时的地面效应扰动问题，提出了基于动态补偿的抗扰控制方法。通过实时更新控制效率矩阵中的推力系数 \(K_{TGE}\) 与舵面偏转力系数 \(K_{cvGE}\)，在控制分配环节实现了对地效影响的精确补偿。静态台架实验表明，补偿后推力与力矩的稳态误差分别控制在 \(3\%\) 与 \(10\%\) 以内。实飞验证进一步证明，引入补偿后无人机在强地效区的高度与偏航角跟踪性能显著提升，稳态误差快速收敛且动态品质一致。所提方法有效抑制了地面效应对飞行控制的不利影响，为涵道式无人机安全起降与低空悬停提供了可靠保障。
...
针对上述问题，非线性控制方法成为提升涵道式无人机全包线飞行品质的必然选择。目前学术界广泛采用的非线性控制策略包括反步控制、滑模控制、模型预测控制以及非线性动态逆控制等。其中，增量式非线性动态逆控制（Incremental Nonlinear Dynamic Inversion, INDI）因其对模型误差和外部扰动具有较强鲁棒性而受到关注\cite{Cheng2024INDI}。然而，INDI控制器依赖于高带宽的角加速度反馈，且对传感器噪声较为敏感，而单螺旋桨涵道式无人机的震动较大，传感器噪声难以被忽略；在模型精度较高的前提下，其性能优势并不明显。相比之下，非线性动态逆控制（Nonlinear Dynamic Inversion, NDI）直接基于系统精确模型进行反馈线性化，能够将原始非线性系统映射为期望的线性解耦形式，具有响应速度快、控制精度高、结构清晰等优点。由于本文已通过第三章的在线参数辨识获得了较为准确的气动参数模型，具备部署NDI控制器的先验条件，因此选用NDI作为开放环境下的抗扰控制器，并在仿真环境中验证其效果。
...
\subsection{NDI控制器与PID控制器对比实验设计}

为验证NDI控制器对非线性气动扰动的抑制能力，本小节在第三章所建立的数值仿真环境中开展对比实验。第三章的仿真平台已集成了涵道式无人机的六自由度非线性动力学模型、传感器噪声模型以及一套串级PID控制器。在此基础上，额外部署了第\ref{sec:NDI_intro}节所设计的NDI控制器，并保持相同的飞行任务与扰动条件，以公平评估两种控制器的性能差异。

需要说明的是，NDI控制器的实际性能高度依赖于模型精度与状态估计的准确性。为使仿真尽可能贴近真实飞行条件，NDI控制律中的非线性函数 \(\boldsymbol{f}_{\eta}(\boldsymbol{x})\) 与控制效率矩阵 \(\boldsymbol{G}_{\eta}(\boldsymbol{x})\) 并非直接使用仿真模型中的真实状态 \(\boldsymbol{x}\)，而是采用第三章扩展卡尔曼滤波器实时估计出的状态 \(\hat{\boldsymbol{x}}\)。这模拟了实际飞控系统中状态观测器输出存在估计误差的情况，能够更真实地反映NDI控制器在工程应用中的鲁棒性。
...
3. 偏航角阶跃响应：依次轮流给定期望偏航角 \(\psi_d\) 为 \(30^\circ\) 和 \(0^\circ\) 的阶跃指令，每个指令维持 \(5\,\mathrm{s}\)，重复三次。
 ...
 \subsection{NDI控制器与PID控制器对比实验数据分析}
...
\begin{figure}[htbp]
    ...
    \end{minipage}
    \caption{PID控制器和NDI控制器横滚角$\varphi$跟随曲线对比}
    \label{fig:angle_cmp}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{minipage}
    \caption{PID控制器和NDI控制器横滚角$\varphi$跟随实验控制力矩对比}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{1.0\textwidth}
        ...
        \caption*{(c)偏航角$\psi$跟随曲线}
    \end{minipage}
    \caption{PID控制器和NDI控制器偏航角$\psi$跟随曲线对比}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{1.0\textwidth}
        ...
        \caption*{(c)控制力矩在机体系$Z$轴的投影$M_{cv, z}$}
    \end{minipage}
    \caption{PID控制器和NDI控制器偏航角$\psi$跟随实验控制力矩对比}
    \label{fig:effort_cmp_yaw}
\end{figure}

\textbf{横滚角阶跃响应实验分析}

从图\ref{fig:angle_cmp}(a)可以看出，在横滚角阶跃指令下，PID控制器与NDI控制器的横滚角响应速度基本相当，两者均能在约 \(1.5\,\mathrm{s}\) 内完成对 \(20^\circ\) 阶跃指令的跟踪。然而，由于系统固有的非线性耦合特性，PID控制器的表现暴露出两个短板。

其一，姿态通道之间存在显著的交叉耦合。观察图\ref{fig:angle_cmp}(b)，当横滚角发生阶跃变化时，PID控制下的俯仰角出现了约 \(1^\circ\) 的扰动，而NDI控制下的俯仰角则几乎不受影响。这一耦合主要源于涵道风扇高速旋转产生的陀螺力矩效应——横滚角速度的变化会诱发俯仰轴上的干扰力矩，而PID控制器缺乏对这种非线性耦合的显式补偿机制，只能被动地依赖姿态环的误差反馈来抑制扰动。相比之下，在已知控制舵面偏转力系数$K_{cv}$的情况下，NDI控制器通过在线求解逆动力学模型，主动在俯仰通道上输出额外的补偿控制力矩，从而提前抵消了陀螺力矩的耦合影响，如图\ref{fig:effort_cmp}(b)中NDI曲线在横滚阶跃瞬间出现的尖峰所示。

其二，偏航通道的初始偏差显著。从图\ref{fig:angle_cmp}(c)可以看到，在仿真初始阶段，PID控制下的偏航角出现了高达 \(30^\circ\) 的偏离，随后才逐渐回归期望值。这是由于第三章参数辨识实验中，反扭矩襟翼系数 \(K_{sta}\) 的初始估计值与真实值存在偏差，导致控制分配模块在初始时刻未能准确抵消涵道风扇反扭矩与襟翼平衡力矩之间的残差。PID控制器需要依赖偏航通道积分项的累积作用来逐步修正这一偏差，因此表现出较长的调节时间。而NDI控制器在控制律中直接利用了当前状态估计 \(\hat{\boldsymbol{x}}\) 中的气动参数信息，能够更准确地计算所需的偏航力矩，从图\ref{fig:effort_cmp}(c)可以看出，NDI输出的 \(M_{cv,z}\) 在初始时刻即与PID输出存在明显差异，这使得偏航角能够迅速稳定在期望值附近，几乎无初始偏移。

\textbf{偏航角阶跃响应实验分析}

从图\ref{fig:angle_cmp_yaw}(c)可以看出，NDI控制器在每次偏航角阶跃指令下的响应曲线几乎完全一致，表现出良好的动态一致性。而PID控制器的响应则存在明显差异：第一次阶跃响应出现约 \(4^\circ\) 的超调，后续阶跃响应的超调量有所变化，且每次调节时间不尽相同。这是由于PID控制器中的积分项具有记忆效应，其数值依赖于前一阶段的误差累积，导致在不同阶跃周期内控制器的初始状态不同，从而影响了响应的重复性。NDI控制器则不存在积分项，其控制量完全由当前状态和模型逆决定，因此能够保证每次阶跃响应具有相同的动态特性。

从图\ref{fig:angle_cmp_yaw}(a)和(b)同样可以观察到PID控制器在横滚与俯仰通道之间的耦合现象，表现与横滚角阶跃实验一致：当偏航角阶跃时，由于偏航运动本身对横滚和俯仰的耦合较弱，俯仰角扰动在 \(0.3^\circ\) 以内，扰动幅度较小，但PID控制下的横滚角仍出现可较为明显的波动，而NDI控制下的横滚与俯仰角则保持平稳。此外，图\ref{fig:angle_cmp_yaw}(c)再次验证了PID控制器在初始阶段的偏航角偏移问题，即前 \(2\,\mathrm{s}\) 存在最大约 \(30^\circ\) 的偏差，而NDI控制器则无此现象，再一次验证了已知反扭矩襟翼系数 \(K_{sta}\)情况下，NDI控制器的优势。

仿真结果表明：在模型精确已知且状态估计可信的前提下，NDI控制器能够有效解耦姿态通道之间的非线性耦合，消除积分项带来的动态不一致性，并利用先验模型信息补偿气动参数偏差引起的初始扰动。相较于PID控制器，NDI控制器在姿态跟踪的快速性、稳定性和一致性方面均具有显著优势。
% 横滚角阶跃信号响应实验：
...
% 此外，从图\ref{fig:angle_cmp}(c)可以看出，初始阶段偏航角难以被控制，出现了高达$30$度的偏差。这是由于部署PID控制器时，在初始阶段对反扭距襟翼的偏转力系数估计不准确，未能准确消除反扭距襟翼和涵道风扇相抵消后，剩余的Z轴扭矩，经过2s的积分项修正后，偏航角姿态才重新回到期望值附近。响应地，从图\ref{fig:effort_cmp}(c)可以看出，PID控制器和NDI控制器之间的输出存在差距，正是对反扭距襟翼的偏转力系数估计不准确引起的。
...
针对近地面飞行时地面效应引起的推力增益与舵面效率衰减问题，本章基于第四章辨识得到的半经验模型，在控制分配环节实时更新推力系数 \(K_{TGE}\) 与舵面偏转力系数 \(K_{cvGE}\)，实现了对地效扰动的动态前馈补偿。静态台架实验表明，补偿后推力与力矩的稳态误差分别控制在 \(3\%\) 与 \(10\%\) 以内；近地面实飞验证进一步证明，引入补偿后无人机在强地效区的高度与偏航角跟踪性能显著提升，稳态误差快速收敛且动态品质一致，有效克服了地效对飞行控制的不利影响。

最后，为应对开放环境中非线性气动耦合与参数不确定性，基于第三章在线辨识得到的关键气动参数，设计了非线性动态逆控制器。仿真对比实验表明，NDI控制器能够主动解耦姿态通道间的陀螺力矩耦合，消除积分项引起的动态不一致性，并利用先验模型信息补偿气动参数偏差带来的初始扰动。相比于串级PID控制器，NDI控制器在姿态跟踪的快速性、稳定性和一致性方面均具有显著优势。

综上，本章所提出的抗扰控制方法兼顾了工程实用性与理论先进性，为涵道式无人机在全飞行包线内的高品质控制提供了系统解决方案。
