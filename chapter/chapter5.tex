...
非线性动态逆控制是一种基于反馈线性化原理的非线性控制方法。考虑一类仿射非线性系统，其状态方程可写为：
\begin{equation}
    \dot{\boldsymbol{x}} = \boldsymbol{f}(\boldsymbol{x}) + \boldsymbol{g}(\boldsymbol{x}) \boldsymbol{u},
\end{equation}
其中 \(\boldsymbol{x}\) 为系统状态向量，\(\boldsymbol{u}\) 为控制输入向量，\(\boldsymbol{f}(\cdot)\) 和 \(\boldsymbol{g}(\cdot)\) 为光滑的非线性函数。NDI控制的核心思想是：选择合适的输出变量 \(\boldsymbol{y} = \boldsymbol{h}(\boldsymbol{x})\)，并通过求解该输出变量与输入之间的微分关系，构造控制律使得闭环系统呈现期望的线性动态特性。

对于本文所研究的涵道式无人机，通常选取姿态角 \(\boldsymbol{\eta} = [\phi, \theta, \psi]^T\) 作为被控输出。对输出求导直至控制输入显式出现，可得：
\begin{equation}
    \ddot{\boldsymbol{\eta}} = \boldsymbol{\alpha}(\boldsymbol{x}) + \boldsymbol{\beta}(\boldsymbol{x}) \boldsymbol{u},
\end{equation}
其中 \(\boldsymbol{\beta}(\boldsymbol{x})\) 称为解耦矩阵，在飞行器的有效工作范围内通常可逆。NDI控制律设计为：
\begin{equation}
    \boldsymbol{u} = \boldsymbol{\beta}^{-1}(\boldsymbol{x}) \left( \boldsymbol{\nu} - \boldsymbol{\alpha}(\boldsymbol{x}) \right),
\end{equation}
式中 \(\boldsymbol{\nu}\) 为伪控制量，通常根据期望的闭环动态选取，如 \(\boldsymbol{\nu} = \ddot{\boldsymbol{\eta}}_d + \boldsymbol{K}_d \dot{\boldsymbol{e}} + \boldsymbol{K}_p \boldsymbol{e}\)，其中 \(\boldsymbol{e} = \boldsymbol{\eta}_d - \boldsymbol{\eta}\) 为姿态跟踪误差，\(\boldsymbol{K}_p\)、\(\boldsymbol{K}_d\) 为增益矩阵。代入后，闭环系统退化为线性二阶系统：
\begin{equation}
    \ddot{\boldsymbol{e}} + \boldsymbol{K}_d \dot{\boldsymbol{e}} + \boldsymbol{K}_p \boldsymbol{e} = \boldsymbol{0},
\end{equation}
从而可以通过极点配置或线性二次型调节等方法设计期望的动态响应。

NDI控制器的优势在于：当系统模型足够精确时，它能够完全抵消系统的非线性项，实现大范围内的全局线性化和解耦控制，响应速度快、稳态精度高。然而，其性能高度依赖于模型精度，对未建模动态和外部扰动较为敏感。在本文中，第三章辨识得到的气动参数为NDI控制器提供了较为精确的先验模型，使其能够充分发挥非线性补偿的优势。后续仿真将对比NDI控制器与串级PID控制器在典型机动工况下的控制效果，验证其对非线性气动扰动的抑制能力。
% \section{NDI抗扰控制器}
