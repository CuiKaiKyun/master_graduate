...
\section{涵道式无人机的系统构成}
\subsection{机体结构}
涵道式无人机大致结构如图\ref{fig:body}所示。顾名思义，涵道式无人机的核心结构是螺旋桨以及涵道壁。该无人机具有C4旋转对称性，即机体绕垂直轴旋转90°后，其几何结构与质量分布均与原状态重合。无人机选用直径为7寸的螺旋桨作为内部的涵道风扇。螺旋桨和其外包裹的筒状涵道壁共同构成完整的涵道结构。涵道壁内径略大于螺旋桨直径，用于约束气流，使内部气流保持轴向流动。

为抵抗螺旋桨高速转动产生的反扭距，在螺旋桨的下方装有反扭距襟翼，用于平衡该反扭距，该襟翼为固定襟翼。

反扭距襟翼下方装有四个由舵机控制的可旋转的控制舵面，用于产生控制力矩，对整机进行闭环控制。为了增加控制舵面产生的气动力对无人机质心的力臂，无人机的质心需要往上移，因此，电池、电机和电调被设计在无人机的顶部。
...
\subsection{程序设计}
程序设计指的是基于C++语言设计的部署在微控制器上的嵌入式程序设计。为了对任务进行管理，该程序部署了FreeRTOS实时操作系统。程序主要负责内容如下：遥控器控制指令获取，传感器数据读取，传感器数据预处理，无人机位姿估计，闭环控制器算法处理，输出执行器指令等。具体程序设计如图\ref{fig:software_figure}所示。
\begin{figure}
    \centering
...
\begin{figure}
    ...
    \caption{\label{fig:ductedfan_force}涵道风扇受力分析}
\end{figure}

定义$\Omega$为螺旋桨的角速度大小，$R$为螺旋桨的半径。

定义$\bm{v}_a$为空速，即机体相对于周围气流的速度，$V_a$为其大小。

定义$\bm{v}_a^{||}$为空速$\bm{v}_a$在机体系平面$X_bO_bY_b$上的投影。
...
\begin{equation}
    N_{df} = \frac{1}{2}\rho \pi^2 R^3 \Omega C_{N_J} V_a sin\alpha_{df}.
\end{equation}
注意到，$V_a sin\alpha_{df}$即为$\bm{v}_a^{||}$的模，且$N_{df}$的方向
与$\bm{v}_a^{||}$方向相反，那么侧向力矢量$\bm{N}_{df}^b$在机体系下的投影可写为：
\begin{equation}
    \bm{N}_{df}^b = \frac{1}{2}\rho \pi^2 R^3 \Omega C_{N_J} \bm{v}_a^{||}.
\end{equation}

已知风扇拉力的方向是指向机体系坐标轴$O_bZ_b$的负方向，那么结合式\ref{eq:thrust}和式\ref{eq:thrust_k}，风扇拉力矢量$\bm{T}_{df}^b$在机体系下的投影可写为：
\begin{equation}
    \bm{T}_{df}^b = \frac{1}{2}\rho \pi R^4 \Omega^2 (C_{T0} + J (C_{T_{J0}} + C_{T_J} cos\alpha_{df})).
\end{equation}

至此，涵道风扇受到的气动力合力已呼之欲出，其在机体系下的投影$\bm{F}_{df}^b$为：
\begin{equation}
...
气动力矩是指涵道风扇的气动力给机体带来的的力矩，从图\ref{fig:ductedfan_force}不难看出，涵道风扇的拉力和侧向力并非经过质心，这些气动力的等效作用点位置的影响因素更为复杂。Ohanian等人所提出的模型是基于侧向力等效作用点不变的假设\cite{2012Nondimensional}，通过较为简约的解析式描述了涵道风扇气动力矩$M_{df}$：
\begin{equation}
    M_{df} = -l_N N_{df} + l_T T_{df}.
\end{equation}
式中，气动力矩$M_{df}$以使无人机具有抬头趋势的方向为正，$X_J$和$X_{\alpha}$均为无量纲的常系数，$l_N$和$l_T$分别是侧向力和拉力对机体重心的力臂，其关系如图\ref{fig:ductedfan_force}所示。该理论认为侧向力的力臂$l_N$不变，而拉力力臂$l_T$满足如下关系：
\begin{equation}
    l_T = X_J J R sin(X_\alpha \alpha_{df}).
\end{equation}

已知力和力臂，即可求出涵道风扇的气动力矩在机体系下的投影$\bm{M}_{df}^b$为：
\begin{equation}
    \bm{M}_{df}^b = l_N \bm{e}_3 \times \bm{N}_{df}^b + l_T \frac{\bm{v}_a^{||}}{\lVert \bm{v}_a^{||} \rVert} \times \bm{T}_{df}^b.
\end{equation}
...
\begin{equation}
    \bm{M}_{gyro}^b = J_{fan} (\bm{\Omega} \times \bm{\omega}^b).
    \label{eq:M_gyro}
\end{equation}
式中，$J_{fan}$为涵道风扇对转轴的转动惯量，$\bm{\Omega}$为转轴的角速度，满足$\bm{\Omega} = [0 \quad 0 \quad \Omega]^T$。
...
动量理论的理论前提是，假设气体是定常的不可压缩流体，且流体在任何位置速度不会突变。该理论将涵道风扇看作一个均匀的，厚度无限小的激励盘。如图\ref{fig:flow_status}所示，设气流在无穷远处的速度大小为$V_0$，由于涵道口的抽吸作用，气流流入涵道口的过程中速度逐渐增加并以速度$V_1$接触到涵道风扇的上表面，由于假设认为流体速度不会突变，因此，气流在经过激励盘后，速度仍为$V_1$，而气流压力骤增，紧接着，气流在排出涵道的过程中，气压逐渐降低为外界气压，气流速度进一步增加，最终以速度$V_e$经过涵道出口截面，并并被排出。
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{figure/body1.png}
    \caption{\label{fig:flow_status}轴流状态的气动分析}
\end{figure}
注意到无穷远处的速度为$V_0$的气流和涵道出口处速度为$V_e$的气流，它们的气压均为外界气压。设单位时间内，从涵道出风口排出的气体质量为$\dot{m}_{air}$，气体密度为$\rho$，根据动量定理可得，涵道风扇对气体的推力等于气流在单位时间内的动量的增加量，即：
\begin{equation}
    T_{df} = \dot{m}_{air} (V_e - V_0) = \rho S V_1 (V_e - V_0),
    \label{eq:dongliang}
\end{equation}
同时注意到，根据能量守恒定律，涵道风扇推力的功率等于单位时间内气体动能的变化量：
\begin{equation}
...
涵道出口风速$V_e$可近似认为是气流掠过反扭距襟翼的速度，由此便可给出襟翼扭矩的解析表达。襟翼扭矩可以认为各个叶片产生的气动力对机体质心的扭矩的总和。假设单个叶片受到的气动力的压心到螺旋桨转轴的距离为$l_{sta}$，单片叶片的面积为$A_{sta}$，叶片的升力系数为$C_{sta}$，叶片数量为$n_{sta}$，襟翼扭矩$M_{sta}$可以表示为：
\begin{equation}
    M_{sta} = \frac{n_{sta}}{2} \rho A_{sta} C_{sta} l_{sta} V_e^2.
\end{equation}

襟翼扭矩的方向为机体系坐标轴$O_bZ_b$的正方向，恰与风扇反扭距方向相反，则该扭矩在机体系下的投影$\bm{M}_{sta}^b$为：
\begin{equation}
    \bm{M}_{sta}^b = M_{sta} \cdot \bm{e}_3 = \frac{n_{sta}}{2} \rho A_{sta} C_{sta} l_{sta} V_e^2 \cdot \bm{e}_3.
    \label{eq:M_sta}
\end{equation}
...
如图\ref{fig:ctrl_surface}所示，本文所研究的无人机具有四个控制舵面。定义四个舵面旋转的正方向均为沿转轴方向，从靠近机体轴一侧指向远离机体轴一侧。并且，定义旋转正方向与$O_bX_b$轴同向的舵面为1号舵，反向的舵面为3号舵；定义旋转正方向与$O_bY_b$轴同向的为2号舵，反向的为4号舵。，定义控制舵面的偏转角度向量为$\bm{\delta} = [\delta_1 \quad \delta_2 \quad \delta_3 \quad \delta_4]^T$，分别对应1，2，3，4号控制舵面。
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{figure/body1.png}
...
如图\ref{fig:ctrl_surface}所示，设1号控制舵面的气动压力的压心为$O_cs$，$O_cs$到机体轴的距离是$l_1$，$O_cs$到机体轴$O_bX_b$的距离是$l_2$，设控制舵面的面积为$A_cv$，控制舵面的升力系数为$C_cv$，且根据对称关系，可给出控制舵面受到的气动力为：
\begin{equation}
    F_i = \frac{1}{2}\rho A_cv C_cv V_e^2, \quad i=1,2,3,4.
\end{equation}
...
根据上述推导，可得出控制舵面的作用力在机体系下的投影$\bm{F}_{vane}^b$为：
\begin{equation}
    \bm{F}_{vane}^b = 
    \begin{bmatrix}
        F_4 - F_2 \\
...
\section{实验样机}
在本文的研究中涉及到模型参数辨识以及基于模型的抗扰动控制，其本质上是在利用模型构建一个对现实物理系统的“反向映射”或“逆动力学”，模型的精度直接决定了这一映射的保真度与有效性。因此需要要在开展正式研究前，获取非线性系统的相关参数。经过大量测试、仿真和验证，本文研究的涵道式无人机相关系统参数被整理成表\ref{tab:F_and_M}。
\begin{table}
    \caption{\label{tab:F_and_M}实验样机非线性系统模型参数}
    \centering{}%
    \begin{tabular}{cccc}
    \toprule
    ...
    $J_fan(kg \cdot m^2)$ & $2.084 \times 10^{-5}$ & $l_N(m)$ & $0.012$\tabularnewline

    $\rho (kg/m^3)$ & $1.225$ & $\sigma _d$ & $0.7800$\tabularnewline
    
    $k_{flap}$ & $6.966 \times 10^{-6}$ & $k_{T0}$ & $7.041 \times 10^{-6}$\tabularnewline
    
    $k_{Th}$ & $-3.646 \times 10^{-6}$ & $k_{Ts}$ & $-2.247 \times 10^{-5}$\tabularnewline
    
    $k_{Ns}$ & $8.593 \times 10^{-4}$ & $k_{cpx}$ & $7.041 \times 10^{-6}$\tabularnewline
    
    $d_{MS}$ & $1.918 \times 10^{-7}$ & $d_{cs}$ & $0.0048$\tabularnewline
    \bottomrule
    \end{tabular}
\end{table}

尽管前文通过机理分析建立了涵道式无人机的非线性数学模型，并在理想条件下对其动力学特性进行了推导与仿真验证，但必须指出的是，该模型本质上是对真实物理系统的一种理想化抽象与近似描述。在实际工程应用中，由于模型简化、装配误差和机械磨损，理论模型与实物系统之间往往存在一定的偏差。
