...
根据几何关系，并结合式\eqref{eq:F_cv}可得出控制舵面的作用力在机体系下的投影$\bm{F}_{cv}^b$为：
\begin{equation}
    \bm{F}_{cv}^b = 
    \begin{bmatrix}
        F_{cv_{4}} - F_{cv_{2}} \\
    ...
    \label{eq:F_cv}
\end{equation}
控制舵面的作用力矩在机体系下的投影$\bm{M}_{cv}^b$为：
\begin{equation}
    \bm{M}_{cv}^b = 
    \begin{bmatrix}
...
\begin{equation}
    \begin{cases}
        \bm{F}^b = \bm{F}_{df}^b + \bm{F}_{cv}^b \\
        \bm{M}^b = \bm{M}_{df}^b + \bm{M}_{fan}^b + \bm{M}_{gyro}^b + \bm{M}_{sta}^b + \bm{M}_{cv}^b
    \end{cases}.
\end{equation}
...
\begin{table}
    ...
    $\bm{F}_{cv}^b$ & 所有控制舵面受到的气动力的合力\tabularnewline
    ...
    $\bm{M}_{cv}^b$ & 所有控制舵面受到的气动力对机体的合扭矩\tabularnewline
    \bottomrule
    \end{tabular}
\end{table}
