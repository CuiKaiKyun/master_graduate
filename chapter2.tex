...
已知风扇拉力的方向是指向机体系坐标轴$O_bZ_b$的负方向，那么结合式\eqref{eq:thrust}和式\eqref{eq:thrust_k}，风扇拉力矢量$\bm{T}_{df}^b$在机体系下的投影可写为：
\begin{equation}
...
根据几何关系，并结合式\eqref{eq:F_cv}可得出控制舵面的作用力在机体系下的投影$\bm{F}_{vane}^b$为：
\begin{equation}
    \bm{F}_{vane}^b = 
    \begin{bmatrix}
        F_{cv_{4}} - F_{cv_{2}} \\
...
至此，根据现有主流理论，涵道式无人机受到的主要的力和力矩均逐一分析完成。式\eqref{eq:base_dynamic}中的$\bm{F}^b$和$\bm{M}^b$可以认为是这些力和力矩的合成，那么在机体系下，有：
\begin{equation}
    \begin{cases}
